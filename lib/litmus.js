...
TestRun.prototype.async = function (desc, func, asyncTimeout) {
    ...
    if (func) {
        var runner = function () {
            func.call(run, handle);
        };
        process && process.nextTick ? process.nextTick(runner) : setTimeout(runner, 0);
    }
    var deprecationNoticeEmitter = function () {
        throw new Error(
            'return value from TestRun.prototype.async is deprecated ' +
            '(to ensure we can provide the option to serialise asynchronous ' +
            'tests for debugging).'
        );
    };
    return {
        finish:     deprecationNoticeEmitter,
        on:         deprecationNoticeEmitter,
        _fireEvent: deprecationNoticeEmitter
    };
};
...
var AsyncHandle = function (desc, timeout) {
    ...
    this._timeout = setTimeout(function () {
        ...
        handle.finished.resolve(new Error('async operation "' + desc + '" timed out after ' + timeout + ' seconds'));
    }, timeout * 1000);
